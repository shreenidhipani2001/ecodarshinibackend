// Payload CMS Integration Utility with Caching
...
// In-memory cache for images (TTL: 30 minutes)
...
export const getImageById = async (imageId) => {
  try {
    // Skip invalid IDs (Payload CMS uses 24-char MongoDB ObjectIds)
    ...
    const fetchPromise = (async () => {
      try {
        ...
        // Handle rate limiting - cache a retry marker
        if (res.status === 429) {
          console.warn(`Rate limited for image ${imageId}`);
          // Cache null for 1 minute to avoid hammering the API
          imageCache.set(imageId, { data: null, timestamp: Date.now() - CACHE_TTL + 60000 });
          return null;
        }
        ...
        const imageData = {
          ...
          thumbnail: data.sizes?.thumbnail?.url ? getImageUrl(data.sizes.thumbnail.url) : getImageUrl(data.url),
          card: data.sizes?.card?.url ? getImageUrl(data.sizes.card.url) : getImageUrl(data.url),
          full: data.sizes?.full?.url ? getImageUrl(data.sizes.full.url) : getImageUrl(data.url),
          alt: data.alt || '',
        };
    ...
    console.error(`Error in getImageById ${imageId}:`, err.message);
    return null;
  }
};
...
export const attachImagesToProducts = async (products) => {
  ...
  // Pre-fetch all unique images (with rate limiting)
  // This ensures each image is fetched only once, even if used by multiple products
  await Promise.all(
    [...allImageIds].map(id => limit(() => getImageById(id)))
  );

  // Now attach images to each product (will use cache - instant)
  return Promise.all(
